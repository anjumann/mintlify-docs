---
title: 'Git Guide'
description: 'Conventions and workflows for using Git with the HeyHomie application'
---
...
<Steps>
  ...
  <Step title="Generate and Add SSH Key (recommended)">
    ```bash
    # Generate SSH key
...
<Checklist>
  ...
  - As a Reviewer, Provide Constructive Feedback
...
<Note>
  Most merges should be done through the GitHub/GitLab interface using the "Squash and merge" option.
</Note>
...
<AccordionGroup>
  <Accordion icon="layer-group" title="Stashing Changes">
    ```bash
    # Stash your changes
    git stash
  ...
  <Accordion icon="code-compare" title="Rebasing">
    ```bash
  ...
  <Accordion icon="hammer" title="Amending Commits">
    ```bash
  ...
  <Accordion icon="cherry" title="Cherry-picking">
    ```bash
...
<AccordionGroup>
  <Accordion icon="rotate-left" title="Undo Last Commit (But Keep Changes)">
    ```bash
    git reset --soft HEAD~1
    ```
  </Accordion>
  
  <Accordion icon="trash" title="Discard All Local Changes">
    ```bash
    git reset --hard HEAD
    ```
  </Accordion>
  
  <Accordion icon="bug" title="Find Which Commit Introduced a Bug">
    ```bash
    git bisect start
